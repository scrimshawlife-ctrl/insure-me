...
### A-047 Privacy-rights end-to-end rehearsal
A disposable local Supabase database MUST run a deterministic privacy-rights rehearsal through the existing production application services and persistence RPCs, not mocked service boundaries. The rehearsal MUST cover privacy request intake, requester identity verification, discovery/export creation and download, correction execution, restriction execution, deletion execution, downstream vendor propagation success/failure, retention-disposition closure, legal-hold blocking before retention with no destructive mutation, append-only audit immutability, idempotency/integrity replay, cross-tenant isolation, independent export-integrity tamper failure, missing/incompatible retention-policy evidence failure, exact persisted policy versions, and policy binding. It MUST exercise exactly these seven request negative paths: unknown/inactive tenant host, failed identity verification, unverified request execution, cross-tenant host access, invalid export credential, policy/configuration binding mismatch, and idempotency/integrity mismatch. CI MUST upload `privacy-rights-rehearsal-report-v1.json` on success or failure with this exact top-level allowlist only: `schemaVersion`, `contractVersions`, `syntheticFixture`, `workflowStates`, `aggregateCounts`, `downstreamStatus`, `timing`, `errorCode`, and `verdict`. The artifact MUST NOT contain `task`, `checks`, hashes, public references, raw exception/error messages, requester names, emails, phones, addresses, quote/driver/vehicle row payloads, raw export contents, status tokens, lookup hashes, encrypted payloads, vendor evidence refs, or Supabase secrets. Passing this synthetic rehearsal MUST NOT claim legal/compliance approval, live requester verification, live vendor fulfillment, hosted Supabase production behavior, or production SLA compliance.

### A-048 FCRA/adverse-action synthetic rehearsal
A disposable local Supabase database MUST run a deterministic adverse-action rehearsal through the existing `createAdverseActionCase`, `recordAdverseActionHandoff`, and `deliverAdverseActionNotice` production services and checked RPCs, not mocked service boundaries. The rehearsal MUST use `psql` only for synthetic fixture setup and aggregate assertions. It MUST cover positive exact CarrierDecision and ExternalReport provenance, explicit owner and ownership-policy snapshot, separate handoff, exact NoticeDefinition version/content hash, exact delivery adapter/policy descriptor, explicit delivered evidence, and append-only audit evidence. It MUST exercise exactly these nine negative paths: cross-tenant/case mismatch, missing contributing report, determination idempotency replay with changed input, delivery before handoff, notice content-hash mismatch, adapter/policy mismatch, live deployment with synthetic notice configuration, delivery idempotency replay with changed input, and direct evidence mutation. CI MUST upload `adverse-action-rehearsal-report-v1.json` on success or failure with this exact top-level allowlist only: `schemaVersion`, `contractVersions`, `syntheticFixture`, `workflowStates`, `aggregateCounts`, `negativePaths`, `timing`, `errorCode`, and `verdict`. The artifact MUST be aggregate and PII-free, and MUST NOT contain hashes, raw IDs beyond contract/version labels, notice bodies, report payloads, names, emails, phones, addresses, exception stacks, secrets, JWTs, or delivery evidence refs. Passing this synthetic rehearsal MUST NOT claim legal/compliance approval, live CRA/provider authorization, carrier adverse-action ownership approval, live notice delivery, hosted Supabase production behavior, or production SLA compliance.

### A-049 Independent security assessment readiness
Externally supplied independent-assessment metadata MUST be accepted only through a fail-closed evidence intake gate bound to the exact selected deployment and TenantConfiguration/deployment version being assessed. The gate MUST require an independent assessor attestation, aggregate finding counts by severity and status only, and exact scope categories covering the canonical threat model, application runtime, authentication/session boundary, tenant isolation/RLS, regulated data flows, provider/carrier boundaries, secrets/configuration, CI/CD supply chain, infrastructure hosting/networking, and observability/audit/incident response. CI MUST always produce and upload `security-assessment-readiness-report-v1.json` with this exact top-level allowlist only: `schemaVersion`, `contractVersions`, `selectedDeployment`, `assessorAttestation`, `scopeCategories`, `aggregateFindingCounts`, `externalEvidence`, `timing`, `errorCode`, and `verdict`. The artifact MUST NOT contain raw findings, PII, secrets, credentials, tokens, payloads, endpoints, report bodies, exploit details, stack traces, subject identifiers, QuoteCase/Driver/Vehicle data, or provider/carrier evidence refs. Completion MUST fail closed when any critical or high finding remains open, when selected-deployment binding is absent, when independent assessor attestation is absent, when required scope categories are incomplete, or when external assessment evidence is missing. Passing this readiness gate MUST NOT claim that T908 is complete, that an independent assessment has occurred, that findings are remediated, or that production security is certified; the only valid verdicts are `READY_FOR_ASSESSMENT` and `BLOCKED`.

### A-051 Critical/high findings disposition readiness
Externally supplied findings-disposition metadata MUST be accepted only through a fail-closed aggregate evidence gate bound to the exact T908 selected deployment, TenantConfiguration/deployment version, independent assessor attestation, and finding register. The T908 critical/high baseline counts MUST reconcile exactly to the disposition packet. The gate MUST require remediation-evidence presence, independent retest coverage, and an independent closure attestation exactly bound to the selected deployment and `assessmentBinding.findingRegisterRef`. Every assessed critical/high finding MUST reconcile to remediation implemented, independently retested, and closed; open or accepted-risk critical/high findings in either baseline or disposition metadata MUST block readiness. CI MUST always execute the gate and upload `security-findings-disposition-readiness-report-v1.json` with `if: always()` and `if-no-files-found: error`, using this exact top-level allowlist only: `schemaVersion`, `contractVersions`, `selectedDeployment`, `assessmentBinding`, `aggregateDispositionCounts`, `independentClosureAttestation`, `externalEvidence`, `timing`, `errorCode`, and `verdict`. The artifact MUST NOT contain raw findings, finding identifiers, exploit details, endpoints, payloads, PII, credentials, secrets, report bodies, stack traces, raw remediation/retest evidence references, or unvalidated closure-attestation strings; invalid closure attestation references and timestamps MUST be emitted as `UNVERIFIED` on every blocked path. Passing this gate MUST NOT claim that T908 or T909 is complete, that an independent assessment occurred, that remediation was effective in production, or that production security is certified; the only valid verdicts are `READY_FOR_DISPOSITION_REVIEW` and `BLOCKED`.

### A-052 Legal/compliance launch-approval readiness
Externally supplied legal/compliance readiness metadata MUST pass a fail-closed aggregate gate bound to the exact selected deployment and TenantConfiguration ID, tenant, agency, and configuration version. It MUST cover the production compliance-gate approval domains, every governing compliance/control domain, and launch-relevant Q-001 through Q-010 decision blockers. Every approval-domain, control-domain, and open-question entry MUST explicitly repeat `environment`, `deploymentRef`, `tenantConfigurationId`, `tenantId`, `agencyId`, and `configurationVersion`, and the validator MUST compare every field exactly to `selectedDeployment` rather than trust `exactBinding`. Approval-domain and open-question entries MUST use only opaque authorized owner, authority, and evidence references; control-domain entries require only opaque evidence references. Reserved missing/status sentinels, including `UNVERIFIED`, `UNKNOWN`, `BLOCKED`, and `NOT_COMPUTABLE`, are invalid input references, while sanitized blocked artifacts MAY emit `UNVERIFIED`. CI MUST always run the gate and upload `legal-compliance-approval-readiness-report-v1.json` with `if: always()` and `if-no-files-found: error`, using this exact top-level allowlist only: `schemaVersion`, `contractVersions`, `selectedDeployment`, `approvalDomains`, `controlDomains`, `openQuestionBlockers`, `externalEvidence`, `timing`, `errorCode`, and `verdict`. Recursive raw evidence, legal conclusions/opinions, notice or contract bodies, provider/carrier responses, PII, secrets, credentials, payloads, stack traces, invented deadlines, and certification claims are forbidden; every blocked path MUST emit only validated opaque values or `UNVERIFIED`. The only verdicts are `READY_FOR_APPROVAL_REVIEW` and `BLOCKED`. Readiness never means approval, and T911 plus all real approvals remain `UNVERIFIED` until authorized external reviewers approve the exact selected deployment.
...
### A-050 Repository-native security scanner baselines
CI MUST run three repository-native baseline gates for T101: a dependency-free tracked source/config secret scanner, a dependency-free tracked source/config SAST scanner, and a `pnpm audit --prod --audit-level high --json` wrapper. The secret scanner MUST fail on private-key blocks, known live token formats, and public secret environment variable names. The SAST scanner MUST fail on high-signal unsafe execution, DOM injection, shell execution, and TLS-bypass patterns including `eval`, `new Function`, `dangerouslySetInnerHTML`, `shell: true`, and disabled TLS verification. Existing explicitly synthetic fixtures MAY be allowed only by the exact local `security-scan: allow-synthetic` marker on the matched line, never by a path-wide or generic fixture/example suppression. Each gate MUST always write one strict PII-free aggregate report on pass or fail: `secret-baseline-report-v1.json`, `sast-baseline-report-v1.json`, and `pnpm-audit-baseline-report-v1.json`. These artifacts MUST contain aggregate counts and verdict metadata only and MUST NOT contain matched source, secret material, paths, raw advisories, endpoints, stack traces, provider/carrier references, consumer identifiers, or errors. The dependency wrapper MUST emit aggregate vulnerability counts only and MUST fail closed when high or critical production dependency vulnerabilities are reported or the audit cannot complete.
