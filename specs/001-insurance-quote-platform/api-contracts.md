...
### Data-use/retention policy inspection

`GET /api/v1/admin/data-use-policies` returns up to 1,000 exact data-use rule versions for the MFA-authenticated caller's single active agency. It requires `POLICY_ADMIN` and exposes each observation type plus its explicit collect, display, underwriting, rating-submission, carrier-only, prohibited, effective, and retirement state.

`GET /api/v1/admin/retention-policies` returns up to 1,000 exact retention policy versions for the caller's single active agency. It requires MFA plus `POLICY_ADMIN` or `PRIVACY_ADMIN` and exposes configured duration, disposition, hold behavior, certification state, scoped provider/carrier references, authority references, and lifecycle times. Each successful inspection emits an AuditEvent. Neither endpoint infers missing duration or approval, accepts tenant/agency input, returns secrets, or permits mutation. Direct table access is denied and ambiguous eligible agency scope fails closed.
