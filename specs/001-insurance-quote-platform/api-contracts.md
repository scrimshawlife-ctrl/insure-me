...
`GET /api/v1/admin/retention-policies` returns up to 1,000 exact retention policy versions for the caller's single active agency. It requires MFA plus `POLICY_ADMIN` or `PRIVACY_ADMIN` and exposes configured duration, disposition, hold behavior, certification state, scoped provider/carrier references, authority references, and lifecycle times. Neither endpoint infers missing duration or approval, accepts tenant/agency input, returns secrets, or permits mutation. Direct table access is denied and ambiguous eligible agency scope fails closed.
