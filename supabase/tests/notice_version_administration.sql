...
select is(has_function_privilege('authenticated','public.create_notice_definition_version(uuid,text,public.notice_category,text,text,boolean,text,text[],uuid)','EXECUTE'),true,'checked create RPC is callable');
...
create temporary table created_notice as select * from public.create_notice_definition_version(
  'b1100000-0000-0000-0000-000000000001','privacy-notice','INSURANCE_PRIVACY','Privacy notice',
...
select is((select count(*) from public.create_notice_definition_version('b1100000-0000-0000-0000-000000000001','privacy-notice','INSURANCE_PRIVACY','Privacy notice','Canonical privacy body',true,'draft:legal-review',array['NEW_LEGAL_COPY'],'b1400000-0000-4000-8000-000000000001')),1::bigint,'create replay is idempotent');
select throws_ok($$select public.create_notice_definition_version('b1100000-0000-0000-0000-000000000001','privacy-notice','INSURANCE_PRIVACY','Changed title','Canonical privacy body',true,'draft:legal-review',array['NEW_LEGAL_COPY'],'b1400000-0000-4000-8000-000000000001')$$,'22023','NOTICE_ADMIN_IDEMPOTENCY_MISMATCH','create replay cannot change content');
...
select is((select count(*) from public.list_notice_definition_versions('b1100000-0000-0000-0000-000000000001')),1::bigint,'authorized admin lists exact versions');
create temporary table second_notice as select * from public.create_notice_definition_version(
  'b1100000-0000-0000-0000-000000000001','privacy-notice','INSURANCE_PRIVACY','Privacy notice v2',
...
select throws_ok($$select public.list_notice_definition_versions('b1100000-0000-0000-0000-000000000001')$$,'P0002','NOTICE_ADMIN_SCOPE_NOT_FOUND','non-policy user cannot list notices');
select throws_ok($$select public.create_notice_definition_version('b1100000-0000-0000-0000-000000000001','blocked','INSURANCE_PRIVACY','Blocked','Body',false,'blocked:evidence',array['BLOCKED'],'b1400000-0000-4000-8000-000000000006')$$,'P0002','NOTICE_ADMIN_SCOPE_NOT_FOUND','non-policy user cannot create notices');
