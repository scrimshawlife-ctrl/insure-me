...
create or replace function private.notice_admin_context(p_agency_id uuid)
...
begin
  ...
  if v_tenant is null or not private.has_permission(v_tenant, p_agency_id, 'POLICY_ADMIN') then
    raise exception using errcode = 'P0002', message = 'NOTICE_ADMIN_SCOPE_NOT_FOUND';
  end if;
  return query select v_tenant, p_agency_id;
end
$$;
revoke all on function private.notice_admin_context(uuid) from public;
grant execute on function private.notice_admin_context(uuid) to authenticated;

create or replace function private.create_notice_definition_version_impl(
  p_agency_id uuid, p_notice_key text, p_category public.notice_category,
...
begin
  select * into v_ctx from private.notice_admin_context(p_agency_id);
  ...
  v_request_hash := encode(extensions.digest(jsonb_build_array('CREATE_NOTICE_VERSION',
    p_agency_id,trim(p_notice_key),p_category,trim(p_title),p_body_markdown,
    p_required_for_quote,trim(p_evidence_ref),to_jsonb(p_reason_codes))::text,'sha256'),'hex');
  perform pg_advisory_xact_lock(hashtextextended(concat_ws('|',v_ctx.tenant_id,p_agency_id,p_idempotency_key),0));
  perform pg_advisory_xact_lock(hashtextextended(concat_ws('|',v_ctx.tenant_id,p_agency_id,trim(p_notice_key)),0));
  select * into v_event from public.notice_definition_events
    where tenant_id=v_ctx.tenant_id and agency_id=p_agency_id and idempotency_key=p_idempotency_key;
  if found then
  ...
  select coalesce(max(version),0)+1 into v_version from public.notice_definitions
    where tenant_id=v_ctx.tenant_id and agency_id=p_agency_id and notice_key=trim(p_notice_key);
  ...
  values (v_ctx.tenant_id,p_agency_id,trim(p_notice_key),v_version,'DRAFT',p_category,'CA',
  ...
  values (v_ctx.tenant_id,p_agency_id,v_notice.notice_definition_id,'CREATED',trim(p_evidence_ref),
  ...
  values (v_ctx.tenant_id,p_agency_id,'NOTICE_VERSION_CREATED',v_actor,
...
create or replace function private.list_notice_definition_versions_impl(p_agency_id uuid)
returns setof public.notice_definitions language sql stable security definer
set search_path=public,private as $$
  select n.* from private.notice_admin_context(p_agency_id) c
...
revoke all on function private.create_notice_definition_version_impl(uuid,text,public.notice_category,text,text,boolean,text,text[],uuid) from public;
...
revoke all on function private.list_notice_definition_versions_impl(uuid) from public;
grant execute on function private.create_notice_definition_version_impl(uuid,text,public.notice_category,text,text,boolean,text,text[],uuid) to authenticated;
...
grant execute on function private.list_notice_definition_versions_impl(uuid) to authenticated;

create or replace function public.create_notice_definition_version(p_agency_id uuid,p_notice_key text,
...
returns public.notice_definitions language sql security invoker set search_path=public,private as $$
  select private.create_notice_definition_version_impl(p_agency_id,p_notice_key,p_category,p_title,
...
$$;
create or replace function public.list_notice_definition_versions(p_agency_id uuid)
returns setof public.notice_definitions language sql stable security invoker set search_path=public,private as $$
  select * from private.list_notice_definition_versions_impl(p_agency_id)
$$;

revoke all on function public.create_notice_definition_version(uuid,text,public.notice_category,text,text,boolean,text,text[],uuid) from public,anon,authenticated;
...
revoke all on function public.list_notice_definition_versions(uuid) from public,anon,authenticated;
grant execute on function public.create_notice_definition_version(uuid,text,public.notice_category,text,text,boolean,text,text[],uuid) to authenticated;
...
grant execute on function public.list_notice_definition_versions(uuid) to authenticated;
