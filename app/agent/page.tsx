...
export default async function AgentQueuePage() {
  ...
  try {
    ...
    const queue = await listAgentQueue(supabase);
                ...
                <tbody>
                  {queue.map((item) => (
                    <tr key={item.quoteCaseId}>
                      ...
                      <td>
                        {item.blockingIssueCount > 0 ? <span className={styles.blocking}>{item.blockingIssueCount} blocking</span> : item.warningIssueCount > 0 ? <span className={styles.warning}>{item.warningIssueCount} warning</span> : <span className={styles.clear}>No open blockers</span>}
                      </td>
                      <td>{item.sourceChannel}</td>
                      <td>{relativeAge(item.createdAt)}</td>
                      <td>{relativeAge(item.updatedAt)} ago</td>
                    </tr>
                  ))}
                </tbody>
              </table>
